...
#		self.label_20['text'] = 'ch'
	...
	def main(self, event):
		...
		while True:
			...
			k = 1
			# шаг 3
			sigma = round(random.uniform(-1, 0), 2)
			x.append([0, 0])
			x[r+1][0] = round(x[r][0] + h * sigma, 2)
			x[r+1][1] = round(float(((vals[len(vals) - 1] - data[len(data) - 1][0]*x[r+1][0]))/data[len(data) - 1][1]), 2)
			# шаг 4
			w.append(round(self.f(self.entry[18].get(),x[r][0], x[r][1]), 2))
			w.append(round(self.f(self.entry[18].get(),x[r+1][0], x[r+1][1]), 2))
			# шаг 5
			while k < 6:
				while (w[r+1] >= w[r]) and (min(x1) <= x[r+1][0] <= max(x1)) and (min(x2) <= x[r+1][1] <= max(x2)):
					r += 1
					sigma = round(random.uniform(-1, 0), 2)
					x.append([0, 0])
					x[r+1][0] = round(x[r][0] + h * sigma, 2)
					...
					w.append(round(self.f(self.entry[18].get(),x[r+1][0], x[r+1][1]), 2))
				k += 1
			if round(math.fabs(w[r+1] - w[r]), 3) <= epsilonF:
				break
			else:
				h = h * random.random()
				change += 1
		...
		self.label_17['text'] = str(x[w.index(max(w))][0])
		self.label_18['text'] = str(x[w.index(max(w))][1])
		self.label_19['text'] = str(max(w))
		self.label_20['text'] = str(change)
		#print( [max(w), x[w.index(max(w))]] )
...
root.title('Метод с возвратом при неудачном шаге')
